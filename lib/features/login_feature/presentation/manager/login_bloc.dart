...
class LoginBloc extends Bloc<LoginEvent, LoginState> {
  ...
  LoginBloc({required this.loginUseCase, required this.authGoogleUseCase})
      : super(LoginState(checkLoginStatus: CheckLoginInit() ,loginStatus: LoginStatusInit())) {
    ...
    on<CheckLoginRemember>((event, emit) async {
      ...
      if(( userName!= null && userName != "" ) && (password != null && password.trim() != "") ){
        ...
        ResponseError responseError = ResponseError(
          message: "cache provider user name pass is empty",
          alertType: null,
          type: null,
          data: null,
          act: null,
        ...
        emit(state.copyWith(newCheckLoginStatus: CheckLoginError(
            responseError: responseError, loginMapModel: LoginInputParams())
        )
        );
      }
    ...
    on<DoLoginEvent>((event, emit) async {
      ...
      response.fold((ResponseError error) {
        emit(state.copyWith(newLoginStatus: LoginStatusError(error: error,loginMapModel: event.loginInfo)));
        ...
        if (event.isRememberMe) {
          CacheProvider.saveString('refreshToken', loginResponseEntity.refreshToken!);
          CacheProvider.saveString('accessToken', loginResponseEntity.accessToken!);
        }
      });
    });
    ...
    on<LoginWithGoogleEvent>((event, emit) async {
      emit(state.copyWith(newLoginStatus: GoogleStatusLoading()));
      Either<ResponseError, LoginResponseEntity> response =
      await authGoogleUseCase.call(event.googleId);

      response.fold((error) {
        emit(state.copyWith(newLoginStatus: LoginStatusError(error: error,loginMapModel: LoginInputParams())));
