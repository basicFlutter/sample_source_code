...
class DriverLicenseScanner extends StatefulWidget {
  const DriverLicenseScanner({Key? key }) : super(key: key);


  @override
  State<DriverLicenseScanner> createState() => _DriverLicenseScannerState();
}

class _DriverLicenseScannerState extends State<DriverLicenseScanner>  with WidgetsBindingObserver {
    ...
    return MultiBlocProvider(
      ...
      child: Builder(
          builder: (context) {
            _context = context;
            return Scaffold(
              body: BlocListener<OcrDriverLicenseBloc, OcrDriverLicenseState>(
                listener: (context, state) {
                  if(state.ocrDriverLicenseStatus is OcrDriverLicenseCompleted){
                    OcrDriverLicenseCompleted ocrDriverLicenseCompleted = state.ocrDriverLicenseStatus as OcrDriverLicenseCompleted;
                    Navigator.of(context).pop(ocrDriverLicenseCompleted.ocrEntity);
                  }

                  if(state.ocrDriverLicenseStatus is OcrDriverLicenseError){
                    OcrDriverLicenseError ocrDriverLicenseError = state.ocrDriverLicenseStatus as OcrDriverLicenseError;
                    AppUtils.showCustomNotification(context: context, messageType: MessageType.error,
                        message: ocrDriverLicenseError.responseError.message??""
                    );
